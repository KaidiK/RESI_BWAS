...
# Summary
In this section, we evaluate the ESs, power and replicability in ABCD studies using the cross-sectional design (using baseline measures only, and using the second measures only) vs. using the longitudinal design (using the fulll dataset) without bootstrapping.
...
```{r}
set.seed(2023)
nboot = 1000 # this is the number of bootstrapping replicates for RESI confidence interval estimation
...
temp = pbmclapply(var_list, FUN = function(x){
  ...
  # CS analysis (using 1st/baseline measurements)
  ...
  # CS analysis (using 2nd measurements)
  data_2nd = subset(dat_x, eventname == "2-year follow-up")
  fit_2nd = eval(parse(text = paste0("glm(formula = ", y, "~ sex_01 + ns(age_years, 2) + ", x , ", data = data_2nd)")))
  
  resi_obj_2nd = fit_2nd %>% resi(data = data_2nd, nboot = nboot) %>% suppressWarnings()
  resi_2nd = resi_obj_2nd$anova[x, "RESI"]
  resi_2nd_ll = resi_obj_2nd$anova[x, "2.5%"]
  resi_2nd_ul = resi_obj_2nd$anova[x, "97.5%"]
  p_val_2nd = resi_obj_2nd$anova[x, "Pr(>Chisq)"]
  beta_2nd = NA
  beta_se_2nd = NA
  ...
  output = rbind(output,
                 data.frame(Y = y, X = x,
                            ...
                            # 2nd measurement analysis
                            beta_2nd = beta_2nd, beta_se_2nd = beta_se_2nd, resi_2nd = resi_2nd, resi_2nd_ll = resi_2nd_ll, resi_2nd_ul = resi_2nd_ul, 
                            p_val_2nd = p_val_2nd, N_2nd = nobs(fit_2nd),
                            # GEE w/ full data
...
```
### Separating between- and within-subject effects
```{r}
# Compare the between-subject effect with cross-sectional study using baselines
set.seed(2023)
nboot = 1000 # this is the number of bootstrapping replicates for RESI confidence interval estimation
...
temp = pbmclapply(var_list, FUN = function(x){
  ...
  ## Longitudinal analysis
  ## 1. GEE
  dat_x %<>%  arrange(temp_id, age_years) %>% group_by(temp_id) %>% mutate(miss_bl = !("Baseline" %in% eventname)) %>% 
    filter(!miss_bl) %>% 
    mutate(X = !!as.name(x), 
           X_bl = X[eventname == "Baseline"],
           X_change = X - X_bl) %>% ungroup()
  
  fit_full_gee = eval(parse(text = paste0("geeglm(formula = ", y, "~ sex_01 + ns(age_years, 2) + X_change + X_bl, data = dat_x, corstr = 'exchangeable', id = temp_id)")))
  # Type 1 anova (sequential): put the variable of interest as the last term
  gee_resi_obj_full = fit_full_gee %>% resi(data = dat_x, overall = FALSE, coefficients = FALSE, nboot = nboot) %>% suppressWarnings()
  gee_resi_full = gee_resi_obj_full$anova["X_bl", "L-RESI"]
  gee_resi_full_ll = gee_resi_obj_full$anova["X_bl", "L 2.5%"]
  gee_resi_full_ul = gee_resi_obj_full$anova["X_bl", "L 97.5%"]
  gee_p_val_full = gee_resi_obj_full$anova["X_bl", "P(>|Chi|)"]
  gee_beta_full = NA #summary(fit_full_gee)$coefficients[x, "Estimate"]
  gee_beta_se_full = NA #summary(fit_full_gee)$coefficients[x, "Std.err"]
...
temp = do.call(rbind, temp)
saveRDS(temp, paste0("Results/ABCD_CS_L_analysis/between_subject_effects_CS_L_data_analysis_GMV.rds"))
...
for (x in var_list){
  ...
  temp = pbmclapply(outcomes, FUN = function(y){
    ...
    resi_obj_2nd = fit_2nd %>% resi_pe(data = data_2nd) 
    resi_2nd = resi_obj_2nd$anova[x, "RESI"]
    p_val_2nd = resi_obj_2nd$anova[x, "Pr(>Chisq)"]
    beta_2nd = NA
    beta_se_2nd = NA
    ...
    output = rbind(output, 
                   data.frame(Y = y, X = x,
                              # baseline analysis
                              beta_bl = beta_bl, beta_se_bl = beta_se_bl, resi_bl = resi_bl, 
                              p_val_bl = p_val_bl, N_bl = nobs(fit_bl),
                              # 2nd measurement analysis
                              beta_2nd = beta_2nd, beta_se_2nd = beta_se_2nd, resi_2nd = resi_2nd,
                              p_val_2nd = p_val_2nd, N_2nd = nobs(fit_2nd),
                              # GEE w/ full data
                              gee_beta_full = gee_beta_full, gee_beta_se_full = gee_beta_se_full, gee_resi_full = gee_resi_full,
                              gee_p_val_full = gee_p_val_full, 
                              # LMM w/ full data,
                              lmm_beta_full = lmm_beta_full, lmm_beta_se_full = lmm_beta_se_full, lmm_resi_full = lmm_resi_full, 
                              lmm_p_val_full = lmm_p_val_full,
...
temp = do.call(rbind, temp)
saveRDS(temp, paste0("Results/ABCD_CS_L_analysis/between_subject_effects_CS_L_data_analysis_ROI_", x, ".rds"))
...
temp = pbmclapply(var_list, FUN = function(x){
                  ...
                  for (y in outcomes){
                    ...
                    # CS analysis (using 2nd measurements)
                    data_2nd = subset(dat_x, eventname == "2_year_follow_up_y_arm_1")
                    fit_2nd = eval(parse(text = paste0("glm(formula = ", y, "~ sex_01 + ns(age_years, 2) + ", x , ", data = data_2nd)")))
                    resi_obj_2nd = fit_2nd %>% resi_pe(data = data_2nd, overall = FALSE, coefficients = FALSE)  %>% suppressWarnings() 
                    resi_2nd = resi_obj_2nd$anova[x, "RESI"]
                    p_val_2nd = resi_obj_2nd$anova[x, "Pr(>Chisq)"]
                    beta_2nd = NA
                    beta_se_2nd = NA
...
temp = pbmclapply(var_list, FUN = function(x){
                  ...
                  for (y in outcomes){
                    ...
                    ## Longitudinal analysis
                    ## 1. GEE
                    dat_x %<>%  arrange(temp_id, age_years) %>% group_by(temp_id) %>% mutate(miss_bl = !("baseline_year_1_arm_1" %in% eventname)) %>% 
                      filter(!miss_bl) %>% 
                      mutate(X = !!as.name(x), 
                             X_bl = X[eventname == "baseline_year_1_arm_1"],
                             X_change = X - X_bl) %>% ungroup()
                    fit_full_gee = eval(parse(text = paste0("geeglm(formula = ", y, "~ sex_01 + ns(age_years, 2) + ns(mean_FD, df = 3) + X_change + X_bl, data = dat_x, corstr = 'exchangeable', weights = frame_count, id = temp_id)")))
                    gee_resi_obj_full = fit_full_gee %>% resi_pe(data = dat_x, overall = FALSE, coefficients = FALSE)  %>% suppressWarnings()
                    gee_resi_full = gee_resi_obj_full$anova["X_bl", "L-RESI"] 
                    gee_p_val_full = gee_resi_obj_full$anova["X_bl", "P(>|Chi|)"]
                    gee_beta_full = NA
...
saveRDS(temp, "Results/ABCD_CS_L_analysis/between_subject_effects_CS_L_data_analysis_FC.rds")
```
