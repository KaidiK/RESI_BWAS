---
title: "Figure 1 - Partial regression plots for RESI (or L-RESI) for age"
author: "Kaidi Kang"
date: "2023-05-01"
...
## Figure 1: Partial regression plots for RESI (or L-RESI) for age
...
```{r}
# the average effect of SD age on RESI:
plot_data = ES_GMV$ES
plot_data$w = 1/plot_data$L_RESI_se_age
fit = lm(L_RESI_age ~ Design + ns(mean_age, 3) + m2_age + ns(m3_age, 3),
         data = plot_data, weights = w)
summary(fit)
```
...
```{r}
# the % increase in ES by using longituidnal studies
plot_data = ES_GMV$ES
plot_data$w = 1/plot_data$L_RESI_se_age
fit = lm(L_RESI_age ~ Design + ns(mean_age, 3) + ns(m2_age, 3) + ns(m3_age, 3),
         data = plot_data, weights = w)
...
m2_age = 7
m3_age = 0
Design = c("Cross-sectional", "Longitudinal")
new_data = data.frame(mean_age, m2_age, m3_age, Design)
pred = predict(fit, newdata = new_data) %>% round(3)
pred
# percentage ch
(pred[2] - pred[1])/pred[1] * 100
```

```{r}
# the average benefit of using longituidnal design among the 16 long studies
plot_data = ES_GMV$ES
long_data = subset(plot_data, Design == "Longitudinal")
mena_benefit = mean(long_data$L_RESI_age - long_data$CS_RESI_age)

mena_benefit %>% round(3)
```
